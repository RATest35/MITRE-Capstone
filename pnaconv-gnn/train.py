...
# ---------------------------------------------------------
# Build the degree histogram required by the PNA scalers.
# ---------------------------------------------------------
def build_degree_histogram(
    ...
    edge_pairs = [
        ...
        for source_index in allowed_set
        for target_index, _ in dataset.out_neighbors[source_index]
        if target_index in allowed_set
    ]
    ...
    in_degree = degree(edge_index[1], num_nodes=node_count, dtype=torch.long)
    max_degree = int(in_degree.max().item())
    degree_histogram = torch.bincount(in_degree, minlength=max_degree + 1)
...
def main() -> None:
    ...
    degree_histogram = build_degree_histogram(dataset, train_indices)
    sampler_config = SamplerConfig(
